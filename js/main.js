...
function cellClicked(elCell) {
    ...
    switch (piece) {
        ...
        case KNIGHT_WHITE:
        case KNIGHT_BLACK:
            possibleCoords = getAllPossibleCoordsKnight(cellCoord, piece === KING_WHITE);
            break
        case PAWN_WHITE:
        case PAWN_BLACK:
        ...
        case QUEEN_WHITE:
        case QUEEN_BLACK:
            possibleCoords = getAllPossibleCoordsQueen(cellCoord, piece === QUEEN_WHITE);
            break;
        case KING_WHITE:
        case KING_BLACK:
...
function getAllPossibleCoordsRook(pieceCoord, isWhite, res = []) {
    // UNFINISHED BUT WORKING RECURSION!
    // pieceCoord.i--
...
function getAllPossibleCoordsKnight(pieceCoord) {
    ...
    for (var i = 0; i < possibleKnightPoses.length; i++) {
        ...
        if (coords.i < 0 || coords.i > gBoard.length - 1 ||
            coords.j < 0 || coords.j > gBoard[0].length - 1 ||
            !isEmptyCell(coords)) continue
        res.push(coords)
    }
    return res;
}
...
function getAllPossibleCoordsQueen(pieceCoord) {
    return getAllPossibleCoordsBishop(pieceCoord).concat(getAllPossibleCoordsRook(pieceCoord))
}
    ...
    for (var i = pieceCoord.i - 1; i <= pieceCoord.i + 1; i++) {
        ...
        for (var j = pieceCoord.j - 1; j <= pieceCoord.j + 1; j++) {
            if (j < 0 || j > gBoard[0].length - 1 ||
                (i === pieceCoord.i && j === pieceCoord.j) ||
                !isEmptyCell({ i, j })) continue
            res.push({ i, j })
        }
    }
    return res
